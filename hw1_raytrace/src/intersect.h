...
struct ray3f {
    ...
    // Check if value is betwen tmin and tmax
    bool is_value_in_range(const float value) const { return value > tmin && value < tmax; }
};
...
// intersects the scene's surfaces and return the first intersection (used for raytracing homework)
